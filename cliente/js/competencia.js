...
function CompetitionsController() {
	...
	this.loadCompetitions = function (data) {
		...
		data.forEach((element, index) => {
			const divCompetition = $('.competenciaPlantilla').clone().removeClass('competenciaPlantilla');
	...
	this.getCompetition = function (id) {
		...
		$.getJSON(`${server}/competitions/${id}`, data => self.loadCompetition(id, data));
	};
	...
	this.getOptions = function (id) {
		const self = this; // reference to CompetitionsController
		const options = $.getJSON(`${server}/competitions/${id}/movies`, data => {
			console.log(data);
	...
	this.loadOptions = function (id, options) {
		...
		options.movies.forEach((movie, index) => {
			const optionDiv = `#opcion${index + 1}`; // Selecting the div with the option to be loaded
	...
	this.getResults = function (id) {
		const self = this;
		$.getJSON(`${server}/competitions/${id}/results`, data => {
	...
	this.loadResults = function (id, data) {
		...
		data.forEach((element, index) => {
			...
			const resultDiv = `#puesto${index + 1}`;
			...
			const votoOVotos = (element.votos > 1) ? 'VOTOS' : 'VOTO';
